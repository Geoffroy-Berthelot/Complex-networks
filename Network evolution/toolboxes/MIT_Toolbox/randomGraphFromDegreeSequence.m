...
while sum(stubs)>0   % while no more stubs are left to connect
    ...
    if cnt>5                       % if rewiring did not work 5 times
        ...
        if length(ind) == 1
            elr = rewireThisEdge([el; ind(1) ind(1) 1],ind(1),ind(1));
        end
        ...
        if length(ind)>2 || isempty(elr)           % restart algorithm
            fprintf(1,'randomGraphFromDegreeSequence(): restarting ...\n')
            stubs = Nseq;
    ...
    if old_sum==new_sum  % no new nodes have been connected, counter+1
        cnt = cnt+1;
    end
    
    if old_sum~=new_sum  % new connections, restart count
        cnt=0;
    end
    ...
    if n1==n2 % no self-loops (if n1 == n2 the node is pointing to hitself)
        continue;  %pass to next iteration
    end            
    
    if adj(n1,n2)>0 % no double edges
        continue;  %pass to next iteration
    end
