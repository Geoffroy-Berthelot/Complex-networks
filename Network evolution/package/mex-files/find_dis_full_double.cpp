/* NETWORK FILE: find distance between two nodes (double version)  -----
 *
 *  Short CPP-file
 ...
 *  For larger N_0, change type: uint32_t to uint64_t
 *  Using char array and the adjacency matrix as a double *
 *  Usage is:
 *  find_dis_double(adjm, in, out);
 *  where :
 *  'adjm' is the adjacency matrix (a MATLAB type, which is a array of double)
